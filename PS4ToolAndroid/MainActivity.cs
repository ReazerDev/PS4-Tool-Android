    ...
    */
        ...
        private void getFilesInDownload()
        {
            if (permissionGranted)
            {
                ...
                FileInfo[] files2 = di.GetFiles("*.sprx", SearchOption.TopDirectoryOnly);
                ...
                foreach (FileInfo f2 in files2)
                {
                    var msgBox = (new AlertDialog.Builder(this)).Create();
                    msgBox.SetTitle("Version");
                    msgBox.SetMessage("For which Version is the Payload " + f2.Name.ToString() + "?");
                    msgBox.SetButton("1.76", delegate
                    {
                        if (File.Exists(pathToFolder + "/1.76/" + f2.Name.ToString()))
                        {
                            AlertDialog.Builder msg = new AlertDialog.Builder(this);
                            msg.SetTitle("Warning!");
                            msg.SetMessage("File already exists! Overwrite?");
                            msg.SetPositiveButton("Yes", delegate
                            {
                                File.Replace(f2.ToString(), pathToFolder + "/1.76/" + f2.Name.ToString(), null);
                            });
                            msg.SetNeutralButton("No", delegate
                            {
                                msg.Dispose();
                        ...
                        File.Move(f2.ToString(), pathToFolder + "/1.76/" + f2.Name.ToString());
                        File.Delete(f2.ToString());
                    });
                    msgBox.SetButton2("4.05", delegate
                    {
                        if (File.Exists(pathToFolder + "/4.05/" + f2.Name.ToString()))
                        {
                            AlertDialog.Builder msg = new AlertDialog.Builder(this);
                            msg.SetTitle("Warning!");
                            msg.SetMessage("File already exists! Overwrite?");
                            msg.SetPositiveButton("Yes", delegate
                            {
                                File.Replace(f2.ToString(), pathToFolder + "/4.05/" + f2.Name.ToString(), null);
                                File.Delete(f2.ToString());
                            });
                            msg.SetNegativeButton("No", delegate
                            {
                        ...
                        File.Move(f2.ToString(), pathToFolder + "/4.05/" + f2.Name.ToString());
                        File.Delete(f2.ToString());

                    });
                    msgBox.SetButton3("4.55", delegate
                    {
                        if (File.Exists(pathToFolder + "/4.55/" + f2.Name.ToString()))
                        {
                            AlertDialog.Builder msg = new AlertDialog.Builder(this);
                            msg.SetTitle("Warning!");
                            msg.SetMessage("File already existes! Overwrite?");
                            msg.SetPositiveButton("Yes", delegate
                            {
                                File.Replace(f2.ToString(), pathToFolder + "/4.55/" + f2.Name.ToString(), null);
                            });
                            msg.SetNeutralButton("No", delegate
                            {
                                msg.Dispose();
                        ...
                        File.Move(f2.ToString(), pathToFolder + "/4.55/" + f2.Name.ToString());
                    });
                    msgBox.Show();
                }
        ...
        private void getFilesInDirectory()
        {
            if (permissionGranted)
            {
                ...
                {
                    DirectoryInfo di = new DirectoryInfo(pathToFolder + "/" + currentVersion);
                    FileInfo[] files = di.GetFiles("*.bin", SearchOption.TopDirectoryOnly);
                    FileInfo[] files2 = di.GetFiles("*.sprx", SearchOption.TopDirectoryOnly);
                    ...
                    foreach (FileInfo f2 in files2)
                    { 
                        finalFiles.Add(f2.Name.ToString());
                    }
